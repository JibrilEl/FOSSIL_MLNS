...
class FOSSILLossV2(LossFunction):
    ...
    def compute(self, z1, z2, adj, sub_g1, alpha=0.5, reg=0.02, metric="cosine", **kwargs):
        ...
        interest_nodes = torch.LongTensor(list(set(node for subgraph in sub_g1 for node in subgraph))).to(z1.device)
        z1 = torch.index_select(z1, 0, interest_nodes)
        z2 = torch.index_select(z2, 0, interest_nodes)
